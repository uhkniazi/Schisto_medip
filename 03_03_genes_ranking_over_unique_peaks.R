# File: 03_03_genes_ranking_over_unique_peaks.R
# Auth: u.niazi@imperial.ac.uk
# DESC: uses the casette object and unique medip peaks for across lifecycle stages created earlier, 
...
### data loading
## unique lifecycle stages with fisher p.values
oGRLunique = f_LoadObject(file.choose())
...
####### NOTE: adhoc hack
## change variable of oGRconserved to a component of the oGRLunique
names(oGRLunique)
oGRconserved = oGRLunique[['m']]
########
...
s = f_step5(lCas, s)
# plot the results
mOverlap = f_step6(s, main='Overlaps male')
oGRpeaks = s$peaks
head(oGRpeaks)
...
oGRgenes.p = GRanges()
# select the peaks from each smp with lowest p.value
...
hist(oGRgenes.p$neg.log10.fisher.p.value, main='p value distribution male', xlab='neg log10 fisher p.value')
temp = f_step6(f_step5(lCas, oGRgenes.p), main='Overlaps male')

oGRgenes.p = oGRgenes.p[order(oGRgenes.p$neg.log10.fisher.p.value, decreasing = T)]

## create a gff and csv file for these 
r = oGRgenes.p
n = make.names(paste('male_genes_ranking_over_conserved_peaks', date(), 'gff'))
dir.create('Results_scripts/Gff_files', showWarnings = F)
export(r, con=paste('Results_scripts/Gff_files/', n, sep=''), format = 'gff3')
n = make.names(paste('male_genes_ranking_over_conserved_peaks', date(), 'csv'))
